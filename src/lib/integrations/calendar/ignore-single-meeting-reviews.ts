...
export async function ignoreSingleMeetingCalendarReviews(params: {
  orgId: string;
  ...
  for (const row of pendingResult.data ?? []) {
    ...
    meetingCounts.set(email, (meetingCounts.get(email) ?? 0) + 1);
  }

  const emailsToIgnore = [...meetingCounts.entries()]
    .filter(([email, count]) => count === 1 && !ownedEmails.has(email))
    .map(([email]) => email);
  ...
  return { ignoredEmails: emailsToIgnore.length, reviewCount };
}
